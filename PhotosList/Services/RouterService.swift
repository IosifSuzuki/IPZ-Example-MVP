...
class RouterService: NSObject {
    ...
    func pushListPhotosViewController() -> Void {
        let listOfPhotosVC =
            self.getViewController(withIdentifier: NSStringFromClass(PhotoListViewController.self)) as! PhotoListViewController
    ...
    func presentImage(WithModel model: PhotoModel) -> Void {
        let imageVC =  self.getViewController(withIdentifier: NSStringFromClass(ImageViewController.self)) as! ImageViewController
